...
class AdropRewardedAdAdModule: RCTEventEmitter, AdropRewardedAdDelegate {
    ...
    func onAdReceived(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.DID_RECEIVE_AD)
    }

    func onAdFailedToReceive(_ ad: AdropRewardedAd, _ errorCode: AdropErrorCode) {
        sendAdEvent(ad, method: AdropMethod.DID_FAIL_TO_RECEIVE_AD, errorCode: AdropErrorCodeToString(code: errorCode))
    }

    func onAdImpression(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.DID_IMPRESSION)
    }

    func onAdClicked(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.DID_CLICK_AD)
    }

    func onAdWillPresentFullScreen(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.WILL_PRESENT_FULL_SCREEN)
    }

    func onAdDidPresentFullScreen(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.DID_PRESENT_FULL_SCREEN)
    }

    func onAdWillDismissFullScreen(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.WILL_DISMISS_FULL_SCREEN)
    }

    func onAdDidDismissFullScreen(_ ad: AdropRewardedAd) {
        sendAdEvent(ad, method: AdropMethod.DID_DISMISS_FULL_SCREEN)
    }
